  ...
  return (
    <div>
      ...
      <div className="page-body">
        ...
        {activeTab === 'cpt'       && <CodeLinkTab href="/settings#cpt-codes" title="💊 CPT Codes" desc="Manage CPT codes in Settings → CPT Codes section." />}
        {activeTab === 'icd'       && <CodeLinkTab href="/settings#icd10-codes" title="🏷️ ICD-10 Codes" desc="Manage ICD-10 codes in Settings → ICD-10 Codes section." />}
...
function CodeLinkTab({ href, title, desc }: { href: string; title: string; desc: string }) {
  return (
    <div className="card" style={{ textAlign: 'center', padding: 64, maxWidth: 500, margin: '40px auto' }}>
      <div style={{ fontSize: 48, marginBottom: 16 }}>{title.split(' ')[0]}</div>
      <div style={{ fontSize: 20, fontWeight: 700, color: '#374151', marginBottom: 8 }}>{title}</div>
      <div style={{ color: '#64748b', marginBottom: 24 }}>{desc}</div>
      <a href={href} className="btn" style={{ textDecoration: 'none' }}>Open in Settings →</a>
    </div>
  );
}
